...
Clean Architecture に基づく 4 層構成。依存方向は外側 → 内側の一方向。詳細は [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) を参照。

| 層              | ディレクトリ             | 責務                                                |
| --------------- | ------------------------ | --------------------------------------------------- |
| **Domain**      | `domain/`                | 純粋なビジネスルール・型定義。副作用禁止。          |
| **Application** | `api/`, `services/`      | ユースケース実行、状態遷移、副作用の集約。          |
| **Adapter**     | `data/`, `presentation/` | 外部接続（状態管理、UI 描画）。ロジックを持たない。 |
| **Framework**   | `routes/`, `paraglide/`  | SvelteKit 固有、i18n 生成物。                       |
...
- 定数（数値）は `domain/constants/` 配下のみに定義する。
- enum（列挙型）は `domain/enums/` 配下のみに定義する。
- interface / type は `domain/` 配下（`entities/` または `types/`）のみに定義する。
...
- `domain/` 外での enum / interface / type / 定数の定義。
