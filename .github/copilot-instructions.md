...
- 勝負が正常についた場合、履歴の最終行が一つ前とて盤が同じになってしまうため、ターンの終わりで履歴を保存するとして、初期盤面はターン0の履歴として保存するようにする。
...
- classの文字が長すぎて、最大文字数を超えている。formatterの設定を見直せないか確認。
- AIに対戦させると、パネルのコマ上限を超えた操作を普通にしている。GameApiで上限を超える操作を禁止するほか、エラーフォールバックの際のAIの挙動も検討する必要がある。
- devで実行している時は、apiのreq/resのログをinfoで出す様にする。
