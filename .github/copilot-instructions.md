...
- 基本用語の定義を新しいmdに記載してください。
- 六角形のパネルの表示をsvgで実装してください。
- github pages でホスティングされているページのデフォルトのlocaleが日本語になるはずなのに、英語になっている。ボタンを押しても日本語に切り替わらない。要調査。
- 駒が増えると、操作が大変になる。
- 一つのマスに複数の駒がいる場合、移動の表示がわかりにくい。移動の表示方法を工夫する必要がある。
