<script lang="ts">
  ...
  let cost = $derived(pieceType.getCost());
  let canAfford = $derived(currentResources >= cost);
...
<button
  ...
  title={m.piece_cost_tooltip({ piece: pieceType, cost })}
>
  <Icon icon={pieceType.getIconName()} size={20} />
  <span>{m.produce()}</span>
</button>
